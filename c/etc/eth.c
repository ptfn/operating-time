...
static void print_hash(const uint8_t *hash, uint8_t *addr)
{
    unsigned int offset = sprintf((char *)addr, "0x");
    printf("0x");

    for (size_t i = 12; i < 32; i++) {
        offset += sprintf((char *)(addr + offset), "%02x", hash[i]);
        printf("%02x", hash[i]);
    }
    printf("\n");
}
